...
def upload_file(local_path: str | Path, *, file_name: Optional[str] = None) -> tuple[bool, str, dict[str, Any]]:
    """Upload to Drive folder. Returns ``(ok, message, metadata)``."""
    sa_path = _service_account_path()
    folder_id = _folder_id()
    if not sa_path or not folder_id:
        return False, "Drive service account JSON or GOOGLE_DRIVE_FOLDER_ID missing", {}
...
def download_file(file_id: str, dest_path: str | Path) -> tuple[bool, str]:
